...
- **云端共享评价**：课程详情页新增「大家怎么说」区域，所有使用者的评价实时同步到云端数据库（Supabase），一人评价、众人可见；提交评价时可填写昵称。接入方式见 [CLOUD_DATABASE.md](CLOUD_DATABASE.md)，未配置时自动回退本地模式
- **个人课程评价**：每门课程详情页支持星级评分与评语，保存后展示"已评 N★"标记，数据保存在浏览器本地，刷新不丢失
...
- **选修分组筛选**：MSCY（Group I/II）、MSCS（Group I/II）、MSAI（Group I/II）、MSAIFS（四个方向）按培养方案展示分组筛选与学分进度
